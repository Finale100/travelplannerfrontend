...
static fetchOneTrip(id) {
    return fetch(`https://travelplanner100.herokuapp.com/trips/${id}`)
    .then(response => response.json())
  }

static fetchOneAcc(id) {
  return fetch(`https://travelplanner100.herokuapp.com/accommodations/${id}`)
  .then(response => response.json())
}

static fetchOneTicket(id) {
  return fetch(`https://travelplanner100.herokuapp.com/tickets/${id}`)
  .then(response => response.json())
}

static fetchOneExperience(id) {
  return fetch(`https://travelplanner100.herokuapp.com/experiences/${id}`)
  .then(response => response.json())
}

static postFetchTrip(start_date, end_date, name, img_url) {
    return fetch(`https://travelplanner100.herokuapp.com/trips/`, {
      method: "POST",
      body: JSON.stringify({
...
static postFetchAcc(city, start_date, end_date, address, relevant_info, trip_id) {
  fetch(`https://travelplanner100.herokuapp.com/accommodations/`, {
    method: "POST",
    body: JSON.stringify({
      start_date: start_date,
      end_date: end_date,
...
static postFetchExp (name, date, city, address, relevant_info, trip_id) {
  fetch(`https://travelplanner100.herokuapp.com/experiences/`, {
    method: "POST",
    body: JSON.stringify({
      name: name,
      date: date,
...
static postFetchTicket (type_of, departure_date_time, departure_location, arrival_date_time, arrival_location, price, relevant_info, trip_id) {
  fetch(`https://travelplanner100.herokuapp.com/tickets/`, {
    method: "POST",
    body: JSON.stringify({
      type_of: type_of,
...
static patchFetchExp(id, name, date, city, address, relevant_info) {
  ...
  static tripEditPatch(event) {
    ...
    fetch(`https://travelplanner100.herokuapp.com/trips/${id}`, {
      method: "PATCH",
